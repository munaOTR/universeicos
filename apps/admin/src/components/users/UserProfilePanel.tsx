  ...
  const handleSendReminder = async () => {
    ...
    try {
      ...
      if (sent === 0 && result.results?.length > 0) {
        ...
        throw new Error(`Failed: ${firstError}`)
      }
                ...
                <div className="bg-white rounded-xl border border-zinc-100 shadow-sm divide-y divide-zinc-100">
                  ...
                  {user.last_reminder_at && (
                    <div className="flex justify-between p-3 text-sm">
                      ...
                      <span className="font-medium text-zinc-900 text-right">
                        {formatDateTime(user.last_reminder_at)}
                      </span>
                    </div>
                  )}
                </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="space-y-1.5">
                      {reminders.map((r, i) => (
                        <div
                          key={r.id}
                          className="flex items-center justify-between bg-white rounded-lg border border-zinc-100 px-3 py-2 text-xs"
                        >
                          <div className="flex items-center gap-2">
                            <Clock01Icon size={12} className="text-zinc-400" />
                            <span className="text-zinc-600">Reminder #{reminders.length - i}</span>
                            <span className="px-1.5 py-0.5 bg-zinc-100 text-zinc-500 rounded capitalize">
                              {r.trigger_source}
                            </span>
                          </div>
              ...
              <div className="space-y-4">
                ...
                <div className="bg-white rounded-xl border border-zinc-100 shadow-sm divide-y divide-zinc-100">
                  ...
                  <div className="flex justify-between p-3 text-sm">
                    ...
                    <span className="font-mono text-zinc-900 bg-zinc-100 px-2 py-0.5 rounded text-xs">
                      {user.referral_code}
                    </span>
                  </div>
                  <div className="flex justify-between p-3 text-sm">
                        ...
                        >
                          ...
                          <div className="w-[calc(100%-2rem)] md:w-[calc(50%-1.5rem)] bg-zinc-50 p-2 rounded-lg border border-zinc-100">
                            <div className="flex items-center justify-between space-x-2 mb-1">
                              <div className="font-bold text-zinc-900 text-xs capitalize">
                                {log.action_type?.replace('_', ' ')}
                              </div>
                              <time className="font-caveat font-medium text-primary-600 text-[10px]">
                                {new Date(log.created_at).toLocaleDateString()}
                              </time>
                            </div>
                            <div className="text-zinc-500 text-[10px]">
                              {log.details?.description ?? 'System event recorded.'}
                            </div>
                          </div>
                        </div>
                      ))}

-- Migration: 0009_admin_invites.sql
-- Description: Updates handle_new_user to assign primary and granular roles from user_metadata.
...
BEGIN
  ...
  IF NEW.raw_user_meta_data->>'granular_roles' IS NOT NULL THEN
    FOR granular_role_name IN SELECT jsonb_array_elements_text(NEW.raw_user_meta_data->'granular_roles')
    LOOP
      SELECT id INTO granular_role_id FROM public.roles WHERE name = granular_role_name LIMIT 1;
  ...
  -- ── Step 6: Insert into public.waitlist with ALL required columns ───────────
